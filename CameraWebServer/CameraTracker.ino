...
camera_fb_t * fb = NULL;
String selectedObject = "None"; // Variable to store the selected object
...
int servoPosX = 90;
int servoPosY = 90;
enum MovServo {anda, nada};
enum MovServo cualServo = nada;
    ...
    while(true) {
        ...
        if (xSemaphoreTake(fbMutex, portMAX_DELAY) == pdTRUE)  {
          ...
          for (uint32_t i = 0; i < result.bounding_boxes_count; i++) {
              ...
              // Solo continuar si la etiqueta coincide con el objeto seleccionado              
              if (String(bb.label) != selectedObject) {
                  continue;
              }
          ...
          if (cualServo == anda) {
            ...
            if(servoPosX < 0) servoPosX = 0;
            if(servoPosX > 180) servoPosX = 180;
            ...
            if(servoPosY < 45) servoPosY = 45;
            if(servoPosY > 135) servoPosY = 135;
...
void startCameraServer() {
    ...
    config.recv_wait_timeout = 15;          // Tiempo de espera para recibir datos (en segundos)
    config.send_wait_timeout = 15;          // Tiempo de espera para enviar datos (en segundos)
    config.lru_purge_enable = false;        // Deshabilitar purga LRU para mantener conexiones activas
    ...
    if (httpd_start(&stream_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(stream_httpd, &uri_select);
...
"<button onclick='selectOption(\"Mario\")'>Mario</button>"
"<button onclick='selectOption(\"mouse\")'>Mouse</button>"
"</div>"
...
"       stream.style.display = 'none';"
"       stream.src = '';"
"       setTimeout(() => { stream.src = '/stream'; }, 50);"
"   }"
"   if (stream.style.display === 'none') {"
"       stream.style.display = 'block';"
"   } else {"
...
static esp_err_t select_handler(httpd_req_t *req) {
    ...
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK) {        
        char objectName[16];
        if (httpd_query_key_value(query, "object", objectName, sizeof(objectName)) == ESP_OK) {
            selectedObject = String(objectName); // Update the global variable
            Serial.println("Selected Object: " + selectedObject); // Print to Serial Monitor
        }
    }

    const char* response = "Object selection received.";
...
static esp_err_t stream_handler(httpd_req_t *req) {
    ...
    while(true) {
        ...
        // Define bounding box coordinates (for example)
        int x = 100, y = 150, w = 200, h = 300;

        // Add custom header for bounding box
        char bbox_header[100];
        snprintf(bbox_header, sizeof(bbox_header), "X-Bounding-Box: x=%d;y=%d;w=%d;h=%d", x, y, w, h);
        httpd_resp_set_hdr(req, "X-Bounding-Box", bbox_header);
